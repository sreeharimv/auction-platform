...
def next_player():
    ...
    if sequential_auction["current_index"] >= len(sequential_auction["player_sequence"]):
        ...
        if len(unsold_players) > 0 and len(sold_players) < len(auctionable_players):
            ...
            current_auction["current_team"] = ""
            current_auction["status"] = "bidding"
            # Don't broadcast at all - let live view poll for updates
            print(f"DEBUG: new round started, no broadcast sent")
    ...
    current_auction["status"] = "bidding"
    # Don't broadcast at all - let live view poll for updates
    print(f"DEBUG: next_player completed, no broadcast sent")
...
def live_version():
    ...
    resp = jsonify({
        "version": auction_version,
        "current_player_id": current_auction.get("player_id")
    })
    # Prevent caching
